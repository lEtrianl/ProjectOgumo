...
{
    public class RadialFill : VisualElement, INotifyValueChanged<float>
    {
            ...
            set
            {
                ...
                if (this.panel != null)
                {
                    using (ChangeEvent<float> pooled = ChangeEvent<float>.GetPooled(this.m_value, value))
                    {
                        ...
                        label.text = text = (value * 100).ToString();
                    }
                }
                else
                {
        ...
        public new class UxmlTraits : VisualElement.UxmlTraits
        {
            ...
            UxmlFloatAttributeDescription m_height = new UxmlFloatAttributeDescription() { name = "height", defaultValue = 100f };
            ...
            public override void Init(VisualElement ve, IUxmlAttributes bag, CreationContext cc)
            {
                ...
                Texture2D tex = Resources.Load<Texture2D>(ate.overlayImagePath);
                if (tex != null)
                {
                ...
                //ate.style.transitionProperty.value = new List<StylePropertyName>( new StylePropertyName())

                ate.style.alignItems = Align.Center;
                ate.style.justifyContent = Justify.Center;

                ate.label = new();
                ate.label.text = ate.text = (ate.value * 100).ToString();
                ate.label.style.position = Position.Absolute;                
                ate.label.style.fontSize = ate.style.fontSize;
